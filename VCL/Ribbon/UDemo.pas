...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, VCL.TMSFNCRibbon,
  VCL.TMSFNCCustomComponent, VCL.TMSFNCBitmapContainer, VCL.TMSFNCRichEditor,
  VCL.TMSFNCTypes, VCL.TMSFNCUtils, VCL.TMSFNCGraphics, VCL.TMSFNCGraphicsTypes,
  VCL.TMSFNCScrollControl, VCL.TMSFNCRichEditorBase, VCL.TMSFNCPageControl,
  VCL.TMSFNCTabSet, VCL.TMSFNCHTMLText, VCL.TMSFNCToolBar,
  VCL.TMSFNCCustomControl, VCL.TMSFNCStatusBar;
...
type
  TForm130 = class(TForm)
    ...
    TMSFNCRibbon1PageControlPage0ToolBar0: TTMSFNCRibbonToolBar;
    TMSFNCRibbon1PageControlPage0ToolBar1: TTMSFNCRibbonToolBar;
    TMSFNCRibbon1PageControlPage0ToolBar2: TTMSFNCRibbonToolBar;
    ...
    procedure TMSFNCRibbonFormCreate(Sender: TObject);
...
procedure TForm130.DoAlignCenterClick(Sender: TObject);
begin
  re.SetSelectionAttribute(TAlignment.taCenter);
end;

procedure TForm130.DoAlignLeftClick(Sender: TObject);
begin
  re.SetSelectionAttribute(TAlignment.taLeftJustify);
end;

procedure TForm130.DoAlignRightClick(Sender: TObject);
begin
  re.SetSelectionAttribute(TAlignment.taRightJustify);
end;

procedure TForm130.DoAppliedTheme(Sender: TObject; AObject: TObject);
begin
  if Assigned(tbo) then
    tbo.SetTheme(rb.Theme, rb.CustomThemeColor);
end;

procedure TForm130.DoBoldClick(Sender: TObject);
begin
  re.SetSelectionBold(True);
end;

procedure TForm130.DoButtonClick(Sender: TObject);
begin
  if Sender is TTMSFNCRibbonToolBarButton then
    TTMSFNCUtils.Message((Sender as TTMSFNCRibbonToolBarButton).Text + ' Clicked !');
end;

procedure TForm130.DoCopyClick(Sender: TObject);
begin
  re.CopyToClipboard;
end;

procedure TForm130.DoCutClick(Sender: TObject);
begin
  re.CutToClipboard;
end;

procedure TForm130.DoFileButtonClick(Sender: TObject);
begin
  TTMSFNCUtils.Message('File Button Clicked !');
end;

procedure TForm130.DoFontNameClick(Sender: TObject);
begin
  re.SetSelectionFontName((Sender as TTMSFNCRibbonToolBarFontNamePicker).SelectedFontName);
end;

procedure TForm130.DoFontNameSelected(Sender: TObject; AFontName: string);
begin
  re.SetSelectionFontName(AFontName);
end;

procedure TForm130.DoFontSizeClick(Sender: TObject);
begin
  re.SetSelectionFontSize(Round((Sender as TTMSFNCRibbonToolBarFontSizePicker).SelectedFontSize));
end;

procedure TForm130.DoFontSizeSelected(Sender: TObject; AFontSize: Single);
begin
  re.SetSelectionFontSize(Round(AFontSize));
end;

procedure TForm130.DoItalicClick(Sender: TObject);
begin
  re.SetSelectionItalic(True);
end;

procedure TForm130.DoItemSelected(Sender: TObject; AItemIndex: Integer);
begin
  rb.Theme := TTMSFNCRibbonTheme(AItemIndex);
end;

procedure TForm130.DoMoreCommandsClick(Sender: TObject);
begin
  TTMSFNCUtils.Message('More Commands Clicked !');
end;

procedure TForm130.DoPasteClick(Sender: TObject);
begin
...
procedure TForm130.DoRedoClick(Sender: TObject);
begin
  re.Redo;
end;

procedure TForm130.DoUnderlineClick(Sender: TObject);
begin
  re.SetSelectionUnderline(True);
end;

procedure TForm130.DoUndoClick(Sender: TObject);
begin
  re.Undo;
end;

procedure TForm130.TMSFNCRibbonFormCreate(Sender: TObject);
var
  s: string;
...
begin
  ...
  rb.Caption.Text := 'Document 1.txt';

  sc := rb.PaintScaleFactor;

  tb := rb.PageControl.PageContainers[0].AddToolBar('Clipboard');
  tb.ShortCutHint := 'C';
  TTMSFNCUtils.SetFontSize(tb.Font, Round(sc * 11));
...
//  TTMSFNCUtils.SetFontSize(tbo.Font, Round(sc * 11));
  ...
  tbo.AutoSize := False;
  tbo.AutoAlign := False;
  tbo.Width := Round(sc * 115);
  tbo.Height := Round(sc * 85);
  ...
  fnp.Width := fnp.Width + rb.ScalePaintValue(20);

  fsp := tb.AddFontSizePicker;
  fsp.OnFontSizeSelected := DoFontSizeSelected;
  fsp.OnClick := DoFontSizeClick;
  fsp.SelectedItemIndex := 0;
  fsp.Width := rb.ScalePaintValue(40);
  fsp.DropDownWidth := rb.ScalePaintValue(50);
  fsp.LastElement := True;

  btn := tb.AddButton;
  btn.BitmapContainer := TMSFNCBitmapContainer1;
  btn.Bitmaps.AddBitmapName('Bold.png');
  btn.BitmapSize := rb.ScalePaintValue(24);
  btn.Width := rb.ScalePaintValue(30);
  btn.Height := rb.ScalePaintValue(30);
...
procedure TForm130.TMSFNCRichEditor1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  sb.Panels.Items[1].Text := 'Word count: ' + IntToStr(WordCount(re.Text));
end;

procedure TForm130.TMSFNCRichEditor1MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  sb.Panels.Items[0].Text := '<b>Selected text: </b>' +  re.SelectedText;
end;

procedure TForm130.TMSFNCRichEditor1SelectionChanged(Sender: TObject);
begin
  sb.Panels.Items[0].Text := '<b>Selected text: </b>' + re.SelectedText;
end;

function TForm130.WordCount(AText: string): Integer;
begin
